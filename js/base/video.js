	...
	function video_play(e){
		...
		if(vid && vid!=''){
			...
			if(type=='youku') $('<iframe width=100% height='+ht+' src=http://player.youku.com/embed/'+vid+ (autoplay?'?autoplay=true':'') + ' frameborder=0 allowfullscreen></iframe>').css({top:top}).appendTo(box);
				...
				var container=$('<video controls playsinline webkit-playsinline x5-video-player-type="h5"></video>').attr({src:vid,poster:$(this).data('poster')}).css({width:window.innerWidth,height:ht,top:top}).appendTo(box);
